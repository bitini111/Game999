...
local function init(skynet, import)
    ...
    skynet.start = function(start_func)
        ...
        -- skynet.error("start debug: ", source,source:sub(1, 1),source:sub(2),vscdebugaux.abspath(source:sub(2)))
        if source and source:sub(1, 1) == "@" then
            source = vscdebugaux.abspath(source:sub(2))
            -- skynet.error("start debug source: ", source)
            if not source:find("/Server/lib/") then
